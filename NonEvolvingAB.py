...
# -------------------------
def save_results(all_results, overall_best_chromosome, representatives, filename_prefix="NonEvolving_Results"):
    """
    Save all results to files for later use
    
    Saves:
    1. JSON file with aggregated statistics and analysis
    2. Pickle file with the best chromosome (as list)
    3. Text file with formatted analysis output
    """
    ...
    rep_names = list(all_results[0]['rep_scores'].keys())
    rep_stats = {}
    for rep_name in rep_names:
        scores = [result['rep_scores'][rep_name] for result in all_results]
        rep_stats[rep_name] = {
    ...
    chromosome_analysis = analyze_chromosome_data(overall_best_chromosome, representatives)
    ...
    # Save chromosome as pickle (for easy loading in Python)
    ...
    with open(text_filename, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("AXELROD GA EXPERIMENT RESULTS\n")
        f.write("=" * 60 + "\n")
        ...
        f.write("=" * 60 + "\n")
        f.write("REPRESENTATIVE STRATEGY PERFORMANCE (averaged over 30 runs)\n")
        f.write("=" * 60 + "\n")
        for rep_name, stats in rep_stats.items():
            f.write(f"{rep_name:20s}: {stats['mean']:6.1f} (±{stats['std']:4.1f})\n")
        ...
        f.write("Cooperation Rates vs Each Representative:\n")
        f.write("-" * 60 + "\n")
        for rep_name, coop_data in chromosome_analysis['cooperation_rates'].items():
            f.write(f"  {rep_name:20s}: {coop_data['rate']:5.1f}% ")
            f.write(f"({coop_data['coop_count']}/{coop_data['total_moves']} moves)\n")
...
def analyze_chromosome_data(chromosome, representatives):
    """
    ...
    for rep in representatives:
        ...
        cooperation_rates[rep.__class__.__name__] = {
...
# Main GA Loop
# -------------------------
def run_axelrod_ga(seed=None):
    """Run Axelrod's genetic algorithm experiment"""
    if seed is not None:
        random.seed(seed)
    
    # Initialize representatives
    # Replace TitForTat with FirstByFeld for NonEvolving_B
    # Replace FirstByFeld with TitForTat for NonEvolving_A
    representatives = [
        axl.TitForTat(),
        #axl.FirstByFeld(),
        axl.Defector(),
    ...
    for rep in representatives:
        ...
        rep_scores[rep.__class__.__name__] = avg_score
...
def analyze_best_chromosome(chromosome, representatives):
    ...
    print("\n" + "=" * 60)
    print("BEST CHROMOSOME ANALYSIS (Best across all 30 runs)")
    print("=" * 60)
...
def run_multiple_experiments():
    """Run the GA experiment 30 times and aggregate results"""
    ...
    for run in range(NUM_RUNS):
        print(f"\nRun {run + 1}/{NUM_RUNS}...")
        result = run_axelrod_ga(seed=None)
        ...
        print(f"  Final best: {result['best_final_score']:.1f}, Final avg: {result['avg_final_score']:.1f}")
    ...
    # Average best and avg scores per generation
    avg_best_scores = np.mean(all_best_scores, axis=0)
    avg_avg_scores = np.mean(all_avg_scores, axis=0)
    
    # Average representative scores
    print("\nREPRESENTATIVE STRATEGY PERFORMANCE (averaged over 30 runs)")
    print("=" * 60)
    rep_names = list(all_results[0]['rep_scores'].keys())
    for rep_name in rep_names:
        scores = [result['rep_scores'][rep_name] for result in all_results]
        ...
        std_score = np.std(scores)
        print(f"{rep_name:20s}: {avg_score:6.1f} (±{std_score:4.1f})")
    ...
    plt.plot(generations_x, avg_best_scores, 'b-o', label='Best Score (avg)', linewidth=2)
    plt.plot(generations_x, avg_avg_scores, 'k-o', label='Average Score (avg)', linewidth=2)
    
    plt.xlabel('Generations', fontsize=12, fontweight='bold')
    plt.ylabel('Mean Score', fontsize=12, fontweight='bold')
    plt.title(f"Axelrod's Genetic Algorithm\nNon-Evolving Environment (averaged over {NUM_RUNS} runs)", 
    ...
    analyze_best_chromosome(overall_best_chromosome, representatives)
    
    # Save all results to files
    save_results(all_results, overall_best_chromosome, representatives)
